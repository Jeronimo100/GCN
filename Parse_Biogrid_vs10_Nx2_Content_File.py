...
#Let's create mappin ?
""" CHECK IF YOU HAVE 1 FEATURE OR MORE OR LESS """
...
for row_interactor_id in Organism_Interactors_D:
  ...
  Official_Symbol_Interactors_D[row_interactor_id] = list(Official_Symbol_Interactors_D[row_interactor_id])[0]
...
for p in Official_Symbol_Interactors:
#for p in papersD:
...
###############################################################################
###############################################################################
""" PRINT PART OF INPUT: to be avoided. USE LMDB or WRAPPERS WITH PAIRS, instead """ 
...
with open('data/BIOGRID-ALL-3.4.163.tab2.txt', 'r') as f: # open the file for reading
  with open('data/BIOGRID.cites', 'w') as output_file1:
    ...
    for count, line in enumerate(f):
      row = line.strip().split('\t') #.rstrip(" ") #print(row_author_id[0])
      row_interactorA_id = row[1]
      #InteractorA_D[row_interactorA_id].append(row_paper_id)        
      row_interactorB_id = row[2] #.rstrip(" ") #print(row_author_id[0])
