...
def load_data(path="data/Social_Spammer_Samples/", dataset="Social_Spammer"):
    """Load citation network dataset (Social_Spammer only for now)"""
...
##############################################################################
########################### READ MOTHER GRAPH ################################
X0, A0, y0, edges0, features0, mother_G_nodes_ids = load_data(path="data/")
if FILTER == 'localpool':
...
##############################################################################
##############################################################################
with open('results_SSPAMMER_NODESno_EDGESno_FEATURESno_LABELSno_LOADt_TRAINt_TESTt_COST_ACCURACY_DataSize_BatchNo_IterNo_LayersNo.txt', 'a') as output_file:
 ...
 for i in range(TotSampNu): #range(len(ListListsSubgrNodes)) # range(m-1):
    ...
    unlabelled_nodes_idx += [ Nodes_Dict[nodes_id[i]] for i in idx_test ]
    ##############################################################################
    #y_train, y_val, y_test, idx_train, idx_val, idx_test, train_mask = get_splits(train_percentage*nodescount/100, val_percentage*nodescount/100, test_percentage*nodescount/100, y)
    ##y_train0, y_val0, y_test0, idx_train0, idx_val0, idx_test0, train_mask0 = get_splits(y0)
    #X, A, y, features, y_train, y_val, y_test, idx_train, idx_val, idx_test, train_mask = X0, A0, y0, features0, y_train0, y_val0, y_test0, idx_train0, idx_val0, idx_test0, train_mask0
    ...
    ##############################################################################
    # Fit
    with open('Time_Measurements_results_SSPAMMER_NODESno_EDGESno_FEATURESno_LABELSno_LOADt_TRAINt_TESTt_COST_ACCURACY_DataSize_BatchNo_IterNo_LayersNo.txt', 'a') as output_file2:
 ...
 y0_test = np.zeros(y0.shape, dtype=np.int32)
 y0_test[unlabelled_nodes_idx] = y0[unlabelled_nodes_idx]
 test_loss, test_acc = evaluate_preds(preds, [y0_test], [unlabelled_nodes_idx])
 #test_loss, test_acc = evaluate_preds(preds, [y0], [unlabelled_nodes_idx])
 ####################################################################################
