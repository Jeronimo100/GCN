...
#with open('results_CORA_NODESno_EDGESno_FEATURESno_LABELSno_LOADt_TRAINt_TESTt_BatchSize_COST_ACCURACY.txt', 'a') as output_file:
    ...
    """Adj Matrix: adj, Sample_Size = 20, SampleNo = """
"""nodescount = np.shape(np)
all_initial_nodes=np.random.choice(range(nodescount), SampleNo, replace=False)
newadj = np.zeros(Sample_Size,SampleSize)
for i in all_initial_nodes:
  Visited_Nodes = [i]
  eimai_edw = i  
  for j in range(Sample_Size-1):
    neighboors = []
    for i in range(nodescount):
      neighboors.append(adj[eimai_edw,j])
    random_neighboor = np.random.choice(range(neighboors))
    eimai_edw = random_neighboor
    Visited_Notes.append[eimai_edw]
    #
    for i in range(Sample_Size):   # IN EACH MATRIX GO SELECT THE ROWS as indices
        for j in range(Sample_Size):
            newadj[i,j] = adj[a[i],a[j]]
        for j in range(featurescount):
            newfeatures[i,j] = features[a[i],j]
        newtrain_mask[i] = train_mask[a[i]]
        newval_mask[i] = val_mask[a[i]]    
        newtest_mask[i] = test_mask[a[i]]
        for j in range(labelscount):
            newy_train[i,j] = y_train[a[i],j]
            newy_val[i,j] = y_val[a[i],j]
            newy_test[i,j] = y_test[a[i],j] """

""" Visited_Set = Null   # Connected Subgraph to be grown
To_Visit = Initial_Node  # Candidates to visit
 
While Visited < Limit
initial_Node = random.rand() NOT IT Visited_Set
  While Visited < Limit && To_Visit != Null
    n = Choose from To_Visit uar
    Extract n from To_Visit
    Visited_Set.Append(n)
    To_Visit.Append(Neighboors(n) which are not Visited) """

   def Sample_Neighboors(adj,featureMatr,train_mask,y_train,y_val,y_test,a,b,c):
		Visited_Set = 1
		To_Visit_Set = 1
                return adj,featureMatr,train_mask,y_train,y_val,y_test,support,graph,G
    ...
    ##################### CALL ME NOW: SAMPLE THE INITIAL INPUT #####################
